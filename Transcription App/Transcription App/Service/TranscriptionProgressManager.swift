...
class TranscriptionProgressManager: ObservableObject {
    ...
    func updateProgress(for recordingId: UUID, progress: Double) {
        guard progress >= 0 && progress <= 1.0 else {
            print("⚠️ [ProgressManager] Invalid progress value: \(progress) for recording: \(recordingId.uuidString.prefix(8))")
            return
        }
    ...
    func addToQueue(recordingId: UUID, position: Int) {
        guard position > 0 else {
            print("⚠️ [ProgressManager] Invalid queue position: \(position) for recording: \(recordingId.uuidString.prefix(8))")
            return
        }
